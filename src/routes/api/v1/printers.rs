...
pub async fn create_printer(
    ...
    let printer_data = state.db.printer().create(input.name, input.manufacturer, profile::id::equals(claims.profile_id.to_string()), vec![
...
pub async fn get_all_printers(Extension(claims): Extension<UserState>, State(state): State<Arc<AppState>>) -> AppResult<impl IntoResponse> {
    let printers = state.db.printer().find_many(vec![printer::profile_id::equals(claims.profile_id.to_string())]).exec().await?;
    Ok(Response::builder()
...
pub async fn update_printer(
    ...
    match state.db.printer().find_first(vec![printer::id::equals(input.id.to_string()), printer::profile_id::equals(claims.profile_id.to_string())]).exec().await? {
    ...
    let printer_data = state.db.printer().update(printer::id::equals(input.id.to_string()), vec![
