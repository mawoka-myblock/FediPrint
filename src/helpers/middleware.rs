...
use crate::helpers::auth::{check_if_token_was_valid, generate_jwt, InputClaims, read_jwt};
...
pub async fn auth_middleware(
    ...
    let claims = read_jwt(token.clone());
    if claims.is_ok() {
        req.extensions_mut().insert(claims.unwrap().claims);
        return Ok(next.run(req).await);
    }

    let invalid_claims = match check_if_token_was_valid(token) {
        Ok(d) => d,
        Err(_) => return Err(StatusCode::NOT_FOUND),
    };

    let new_jwt = generate_jwt(InputClaims {
    ...
    let auth_cookie = Cookie::build(("authorization_key", new_jwt))
        .secure(true)
        .http_only(true).build().to_string();
